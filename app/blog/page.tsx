...
import { getAllBlogPosts, getBlogCategories, BlogPost } from '@/lib/blog';
...
export default function BlogPage() {
  ...
  const authors = useMemo(() => {
    const authorSet = new Set(allPosts.map(post => post.author));
  ...
  const filteredPosts = useMemo(() => {
    return allPosts.filter(post => {
      ...
      if (selectedCategory) {
        ...
        if (selectedCategory === 'Scalloping' && !postContent.includes('scallop')) return false;
        if (selectedCategory === 'Lobstering' && !postContent.includes('lobster')) return false;
        if (selectedCategory === 'Wildlife' && !postContent.includes('whale')) return false;
        if (selectedCategory === 'Conditions' && 
            !postContent.includes('water') && 
            !postContent.includes('visibility') &&
            !postContent.includes('conditions')) return false;
        if (selectedCategory === 'Seasonal' && 
            !postContent.includes('season') && 
            !postContent.includes('sale')) return false;
      }
  ...
  }, [allPosts, searchQuery, selectedCategory, selectedAuthor]);
